# Original author: Julien Marrec
# Original author email: julien.marrec@gmail.com

# Modifying author: Kingsley Nweye
# Modifying author email: etonwana@yahoo.com

# start the measure
class ResizeExistingWindowsToMatchAGivenWwrCustom < OpenStudio::Ruleset::ModelUserScript

  # human readable name
  def name
    return "Resize existing windows to match a given WWR Custom"
  end
...
The windows will be resized around their centroid but the offset from ceiling is maintained.
...
2. Resize all of the existing windows by re-setting the vertices: scaled centered on centroid but the offset from ceiling is maintained.
    ...
    surfaces.each do |surface|
      ...
      surface.subSurfaces.each do |subsurface|
        ...
        subsurface.vertices.each do |vertex|
          ...
          # runner.registerInfo("Old vertices: #{vertex}")
          z_previous = vertex.z

          # Change the vertex
          vertex = g + centroid_vector
          # runner.registerInfo("New vertices: #{vertex}")
          
          # Shift upwards to maintain offset from ceiling
          vertices << OpenStudio::Point3d.new(vertex.x,vertex.y,vertex.z + (z_previous - vertex.z))
        ...
        # # Check that new vertices are placed correctly
        # subsurface.vertices.each do |vertex|
        #   runner.registerInfo("New vertices after shifting: #{vertex}")
        # end
...
ResizeExistingWindowsToMatchAGivenWwrCustom.new.registerWithApplication
